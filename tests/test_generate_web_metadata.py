...
# These mocks are necessary because these packages might not be installed
# in the environment where the tests are run, and we want to avoid
# heavy dependencies for simple logic tests.
for m in [
    "docling",
    "docling.datamodel",
...
import scripts.generate_web_metadata_recursive as script  # noqa: E402
...
def test_process_folder_colloquium_missing_date(tmp_path):
    ...
    with patch("scripts.generate_web_metadata_recursive.LLMClient") as mock_llm_client_class:
        ...
        with patch(
            "scripts.generate_web_metadata_recursive.llm.get_summary_and_metadata_of_pdf"
        ) as mock_get:
            script.process_folder(str(folder), config_file)
